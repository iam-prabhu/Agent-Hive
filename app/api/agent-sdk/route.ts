...
export async function POST(req: NextRequest) {
    ...
    if (agentDetail?.agentToolConfig?.tools && Array.isArray(tools)) {
      agentDetail?.agentToolConfig?.tools.forEach((t: any) => {
    ...
    let systemPrompt = `You are an AI assistant orchestrator named ${agentDetail?.name}.\n\n`;
    systemPrompt += `You determine which agent to use based on the user query and have access to the provided tools.\n`;

    if (agentDetail?.agentToolConfig?.agents && agentDetail?.agentToolConfig?.agents.length > 0) {
      systemPrompt += `\nYour capabilities incorporate the roles of the following sub-agents:\n`;
      agentDetail?.agentToolConfig?.agents.forEach((agent: any) => {
